# frozen_string_literal: true

appraise "rails-5.1-sidekiq-5.0" do
  gem "activejob", "~> 5.1.0"
  gem "activesupport", "~> 5.1.0"
  gem "sidekiq", "~> 5.0.4"
end

appraise "rails-5.1-sidekiq-5.1" do
  gem "activejob", "~> 5.1.0"
  gem "activesupport", "~> 5.1.0"
  gem "sidekiq", "~> 5.1.0"
end

appraise "rails-5.1-sidekiq-5.2" do
  gem "activejob", "~> 5.1.0"
  gem "activesupport", "~> 5.1.0"
  gem "sidekiq", "~> 5.2.0"
end

appraise "rails-5.1-sidekiq-6.0" do
  gem "activejob", "~> 5.1.0"
  gem "activesupport", "~> 5.1.0"
  gem "sidekiq", "~> 6.0.1"
end

appraise "rails-5.1-sidekiq-6.1" do
  gem "activejob", "~> 5.1.0"
  gem "activesupport", "~> 5.1.0"
  gem "sidekiq", "~> 6.1.0"
end

appraise "rails-5.1-sidekiq-6.2" do
  gem "activejob", "~> 5.1.0"
  gem "activesupport", "~> 5.1.0"
  gem "sidekiq", "~> 6.2.0"
end

appraise "rails-5.1-sidekiq-6.3" do
  gem "activejob", "~> 5.1.0"
  gem "activesupport", "~> 5.1.0"
  gem "sidekiq", "~> 6.3.0"
end

appraise "rails-5.2-sidekiq-5.0" do
  gem "activejob", "~> 5.2.0"
  gem "activesupport", "~> 5.2.0"
  gem "sidekiq", "~> 5.0.4"
end

appraise "rails-5.2-sidekiq-5.1" do
  gem "activejob", "~> 5.2.0"
  gem "activesupport", "~> 5.2.0"
  gem "sidekiq", "~> 5.1.0"
end

appraise "rails-5.2-sidekiq-5.2" do
  gem "activejob", "~> 5.2.0"
  gem "activesupport", "~> 5.2.0"
  gem "sidekiq", "~> 5.2.0"
end

appraise "rails-5.2-sidekiq-6.0" do
  gem "activejob", "~> 5.2.0"
  gem "activesupport", "~> 5.2.0"
  gem "sidekiq", "~> 6.0.1"
end

appraise "rails-5.2-sidekiq-6.1" do
  gem "activejob", "~> 5.2.0"
  gem "activesupport", "~> 5.2.0"
  gem "sidekiq", "~> 6.1.0"
end

appraise "rails-5.2-sidekiq-6.2" do
  gem "activejob", "~> 5.2.0"
  gem "activesupport", "~> 5.2.0"
  gem "sidekiq", "~> 6.2.0"
end

appraise "rails-5.2-sidekiq-6.3" do
  gem "activejob", "~> 5.2.0"
  gem "activesupport", "~> 5.2.0"
  gem "sidekiq", "~> 6.3.0"
end

appraise "rails-6.0-sidekiq-5.2" do
  gem "activejob", "~> 6.0.0"
  gem "activesupport", "~> 6.0.0"
  gem "sidekiq", "~> 5.2.0"
end

appraise "rails-6.0-sidekiq-6.0" do
  gem "activejob", "~> 6.0.0"
  gem "activesupport", "~> 6.0.0"
  gem "sidekiq", "~> 6.0.1"
end

appraise "rails-6.0-sidekiq-6.1" do
  gem "activejob", "~> 6.0.0"
  gem "activesupport", "~> 6.0.0"
  gem "sidekiq", "~> 6.1.0"
end

appraise "rails-6.0-sidekiq-6.2" do
  gem "activejob", "~> 6.0.0"
  gem "activesupport", "~> 6.0.0"
  gem "sidekiq", "~> 6.2.0"
end

appraise "rails-6.0-sidekiq-6.3" do
  gem "activejob", "~> 6.0.0"
  gem "activesupport", "~> 6.0.0"
  gem "sidekiq", "~> 6.3.0"
end

appraise "rails-6.0-sidekiq-6.4" do
  gem "activejob", "~> 6.0.0"
  gem "activesupport", "~> 6.0.0"
  gem "sidekiq", "~> 6.4.2" # above 6.4.2 to ensure we test against breaking changes
end

appraise "rails-6.1-sidekiq-5.2" do
  gem "activejob", "~> 6.1.0"
  gem "activesupport", "~> 6.1.0"
  gem "sidekiq", "~> 5.2.0"
end

appraise "rails-6.1-sidekiq-6.0" do
  gem "activejob", "~> 6.1.0"
  gem "activesupport", "~> 6.1.0"
  gem "sidekiq", "~> 6.0.1"
end

appraise "rails-6.1-sidekiq-6.1" do
  gem "activejob", "~> 6.1.0"
  gem "activesupport", "~> 6.1.0"
  gem "sidekiq", "~> 6.1.0"
end

appraise "rails-6.1-sidekiq-6.2" do
  gem "activejob", "~> 6.1.0"
  gem "activesupport", "~> 6.1.0"
  gem "sidekiq", "~> 6.2.0"
end

appraise "rails-6.1-sidekiq-6.3" do
  gem "activejob", "~> 6.1.0"
  gem "activesupport", "~> 6.1.0"
  gem "sidekiq", "~> 6.3.0"
end

appraise "rails-6.1-sidekiq-6.4" do
  gem "activejob", "~> 6.1.0"
  gem "activesupport", "~> 6.1.0"
  gem "sidekiq", "~> 6.4.2" # above 6.4.2 to ensure we test against breaking changes
end

appraise "rails-6.1-sidekiq-6.4-ddtrace-0.x" do
  gem "activejob", "~> 6.1.0"
  gem "activesupport", "~> 6.1.0"
  gem "sidekiq", "~> 6.4.2"
  gem "ddtrace", "~> 0.54"
end

appraise "rails-6.1-sidekiq-6.4-ddtrace-1.0" do
  gem "activejob", "~> 6.1.0"
  gem "activesupport", "~> 6.1.0"
  gem "sidekiq", "~> 6.4.2"
  gem "ddtrace", "1.0"
end

appraise "rails-6.1-sidekiq-6.4-ddtrace-1.x" do
  gem "activejob", "~> 6.1.0"
  gem "activesupport", "~> 6.1.0"
  gem "sidekiq", "~> 6.4.2"
  gem "ddtrace", "~> 1.0", "< 1.8.0"
end
